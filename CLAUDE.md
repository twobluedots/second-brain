...
uv run streamlit run src/app.py  # Run with uv
```
...
categorize.py       — LLM auto-tagging (OpenAI → Anthropic → Ollama → fallback)
processing.py       — Whisper voice transcription
```

- **src/app.py**: Streamlit UI — dialogs, page rendering, session state
- **src/notes_service.py**: `NoteService` — `save_note()` calls categorize, storage, and event log
- **src/storage/storage.py**: `Storage` — SQLite + ChromaDB, no business logic
- **src/categorize.py**: `categorize(text) -> str` — returns one category from predefined list
- **src/processing.py**: `process_voice_note()` — Whisper transcription
- **src/models/entry.py**: `Entry` Pydantic model — contract between service and storage
